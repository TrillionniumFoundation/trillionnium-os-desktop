...
statistics, state, estimate, and ratio prose remains permitted when it is not
declaration-shaped.

## Bounds and failure behavior

Input retains the one-mebibyte document limit. HTML scanning is linear, quote
aware, does not follow links, and never executes markup. Candidate state retains
only a 64-character visible suffix; invisible padding does not consume that
budget. Unterminated markup stays visible rather than causing the remainder of
the document to disappear from detection.
...
soft-wraps, and long invisible/tag padding. It also verifies that ordinary
multilingual and non-authority prose remains valid. The existing claim-projection
corpus and the authoritative full Python discovery remain mandatory.
