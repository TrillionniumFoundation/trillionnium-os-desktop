...
The public-symbol probes deliberately compile imports rather than instantiate
or object-cast the imported item. This supports structs, enums, type aliases
and non-object-safe traits uniformly while still proving that an external
crate can name each public path.
"""
...
def aggregate_source(symbols: dict[str, str], methods: dict[str, str]) -> str:
    ...
    return f'''// Generated by tools/qualify_servo_exact_pin_v2.py for Servo {PIN}.
#![allow(unused_imports)]
{imports}
...
def main() -> int:
    ...
    args.output_probe.write_text(source)
    manifest = {
