...
script_dir=$(cd -- "$(dirname -- "${BASH_SOURCE[0]}")" && pwd -P) || {
  echo "cannot locate pinned e2fsprogs script directory" >&2
  exit 1
}
# shellcheck source=reject_symlink_path.sh
source "$script_dir/reject_symlink_path.sh"
reject_symlink_path "$manifest" "e2fsprogs manifest" || exit 1
reject_symlink_path "$work_dir" "e2fsprogs work directory" || exit 1
reject_symlink_path "$evidence" "e2fsprogs evidence" || exit 1
...
[[ -f "$manifest" && ! -L "$manifest" ]] || {
  echo "e2fsprogs manifest is missing or unsafe" >&2
  exit 1
}
[[ -d "$work_dir" && ! -L "$work_dir" ]] || {
  echo "e2fsprogs work path is not a regular directory" >&2
  exit 1
}
[[ -d "$(dirname -- "$evidence")" ]] || {
  echo "e2fsprogs evidence parent is not a directory" >&2
  exit 1
}
...
if [[ "$repository" != https://github.com/tytso/e2fsprogs.git ]]; then
  echo "pinned e2fsprogs repository is not the canonical upstream" >&2
  exit 1
fi
