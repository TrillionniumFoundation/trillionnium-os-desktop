#!/usr/bin/env python3
"""D0T-03 governance settings manager, API-compatible revision 2.

This revision is the supported administration entry point. It generates only
GitHub REST fields documented for branch protection, repository rulesets,
workflow permissions, and protected environments. It remains plan-only unless
an operator supplies an authenticated token and exact repository confirmation.
"""
...
def branch_ruleset_payload() -> dict[str, Any]:
    return {
    ...
    def request(
        ...
        request = urllib.request.Request(
            ...
            headers={
                ...
                "User-Agent": "Trillionnium-D0T03-governance-admin-v2",
...
def read_team(client: GitHubClient, organization: str, slug: str) -> Team:
    ...
    logins = tuple(
        sorted(
            ...
            if isinstance(member, dict) and isinstance(member.get("login"), str)
        )
    )
...
def build_plan(
    ...
    return {
        "schema": "trillionnium.desktop.d0t03-governance-plan.v2",
        ...
        "operations": [
            ...
            },
            {
                ...
                "payload": branch_ruleset_payload(),
            },
            {
                "method": "POST",
                "path": f"/repos/{repository}/rulesets",
        ...
        "manual_postconditions": [
            "replace interim individual CODEOWNERS with organization teams",
            "run negative direct-push, force-push, delete, self-review, stale-review, failed-check, and administrator-bypass probes",
            "complete one compliant merge using two different authorized people",
            "prove protected production environment self-review rejection and two-team approval",
            "prove offline release-key custody and source-author separation",
        ],
        "claim_ceiling": {
...
def discover_plan(client: GitHubClient, args: argparse.Namespace) -> dict[str, Any]:
    return build_plan(
        ...
        args.repo,
        args.source_author,
        read_team(client, args.owner, args.security_team),
        read_team(client, args.owner, args.governance_team),
        read_team(client, args.owner, args.release_team),
    )


def apply_plan(
    client: GitHubClient, plan: dict[str, Any], confirmed: str
) -> list[dict[str, Any]]:
...
def main() -> int:
    ...
    try:
        if args.command == "self-test":
            plan = fixture_plan()
            result = {
                "schema": "trillionnium.desktop.d0t03-governance-admin-self-test.v2",
                ...
                "ruleset_payloads_api_field_closed": all(
                    set(operation["payload"]).issubset(
                        {
                            "name",
                            "target",
                            "enforcement",
                    ...
                    for operation in plan["operations"]
                    if operation["path"].endswith("/rulesets")
                ),
                "settings_applied": False,
                "independent_review_completed": False,
                "release_key_custody_proven": False,
            ...
            if args.command == "plan":
                ...
                result = {
                    "schema": "trillionnium.desktop.d0t03-governance-apply-result.v2",
                    "repository": plan["repository"],
                    "operations": apply_plan(
                        client, plan, args.confirm_repository or ""
                    ),
