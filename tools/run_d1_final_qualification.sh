...
# Discover every package-aware repository suite (root, D1, transport, and any
# future suite directories) so this permanent gate cannot silently skip a
# regression added outside the historical D1-only folder.
...
install -D -m 0755 target/release/hepta-agent-port-qualificationd \
  "$RUNNER_TEMP/hepta-agent-port-qualificationd"
cargo build --release --locked \
  -p hepta-agent-portd \
  --no-default-features \
  --features d1-qualification \
...
install -m 0755 "$RUNNER_TEMP/hepta-agent-port-qualificationd" \
  target/release/hepta-agent-port-qualificationd
