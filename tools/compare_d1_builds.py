...
def _has_symlink_component(path: Path) -> bool:
    """Check a raw CLI path before any symlink-resolving normalization."""
...
def _open_regular(path: Path, label: str) -> int:
    """Open one regular input without following a late symlink replacement."""
...
def read_bytes(path: Path, label: str = "D1 input") -> bytes:
...
def read_text(path: Path, label: str = "D1 input") -> str:
    descriptor = _open_regular(path, label)
    try:
...
def load_json(path: Path, label: str = "D1 JSON input") -> dict[str, Any]:
    descriptor = _open_regular(path, label)
    try:
        with os.fdopen(descriptor, "r", encoding="utf-8", closefd=True) as stream:
            descriptor = -1
...
def _write_text_atomic(path: Path, text: str) -> None:
    """Publish a comparison result without following a destination link."""

    if _has_symlink_component(path.parent):
        raise ValueError(f"comparison result parent contains a symlink: {path.parent}")
    path.parent.mkdir(parents=True, exist_ok=True)
    if _has_symlink_component(path.parent) or path.is_symlink():
        raise ValueError(f"comparison result path contains a symlink: {path}")
    ...
    try:
        ...
        if _has_symlink_component(path) or path.is_symlink():
            raise ValueError(f"comparison result path became a symlink: {path}")
...
def sha256(path: Path) -> str:
    return hashlib.sha256(read_bytes(path, "hashed D1 artifact")).hexdigest()


def load_build(artifacts: Path) -> dict[str, object]:
    result_path = artifacts / "build-result.json"
    result = load_json(result_path, "build result")
...
def rootfs_entries(path: Path) -> dict[str, dict[str, Any]]:
    document = load_json(path, "rootfs manifest")
...
def main() -> int:
    ...
    for path, label in (
        (args.first, "first build"),
        (args.second, "second build"),
        (args.prepared_inputs, "prepared inputs"),
        (args.result, "comparison result"),
    ):
        if _has_symlink_component(path):
            raise SystemExit(f"{label} path contains a symlink: {path}")

    first = args.first.absolute()
    second = args.second.absolute()
    prepared_path = args.prepared_inputs.absolute()
    result_path = args.result.absolute()
    if not first.is_dir() or not second.is_dir():
        raise SystemExit("build artifact roots are missing or unsafe")
    if not prepared_path.is_file():
        raise SystemExit(f"prepared inputs are missing or unsafe: {prepared_path}")
    prepared = load_json(prepared_path, "prepared inputs")
    ...
    package_lines = [
        line
        for line in read_text(first / "package-lock.tsv", "package lock").splitlines()
        if line.strip()
    ]
