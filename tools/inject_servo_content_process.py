...
def fail(state: Path, reason: str, diagnostic: str | None = None) -> NoReturn:
...
def proc_state(stat: str) -> str | None:
    end = stat.rfind(")")
    if end < 0:
        return None
    fields = stat[end + 2 :].split()
...
def proc_pids() -> list[int]:
    """Enumerate numeric procfs entries without relying on ``children`` files.

    The optional ``/proc/<pid>/task/<pid>/children`` interface is absent on
    some hosted kernels.  Direct parent-PID matching is both more portable and
    the same topology proof used by the headed runtime itself.
    """
...
def cmdline_args(pid: int) -> list[str]:
    try:
...
def direct_content_processes(runtime_pid: int, runtime_exe: str | None) -> list[int]:
    """Return exact, live content children of the runtime process.

    Every predicate is checked from one procfs snapshot per candidate: direct
    PPID, non-zombie state, canonical executable identity, and an exact argv
    token.  A caller still performs the start-time check immediately before
    signalling, so a disappearing or reused PID cannot receive the fault.
    """
    ...
    for pid in proc_pids():
        if pid <= 1 or pid == runtime_pid:
            continue
        stat = proc_stat(pid)
        if stat is None or parent_pid(stat) != runtime_pid:
            continue
        state = proc_state(stat)
        if state is None or state == "Z" or start_time(stat) is None:
            continue
        if executable(pid) != runtime_exe:
            continue
        if "--content-process" not in cmdline_args(pid):
            continue
        result.append(pid)
...
def main() -> None:
    ...
    # Keep the injector deadline below the headed runtime's 150-second
    # watchdog so a stalled target probe produces injector diagnostics before
    # the runtime masks it with its own timeout failure.
    ...
    last_probe = "no procfs probe completed"
    while time.monotonic() < deadline:
        # The content child exposes its embedder as the direct parent.  Resolve
        # that parent from the identity-bearing process, then enumerate /proc
        # by direct PPID.  Do not depend on the optional procfs `children`
        # interface, which is absent on some hosted kernels.
        stat = proc_stat(expected_pid)
        if stat is not None:
            parent = parent_pid(stat)
            if parent is not None:
                runtime_exe = executable(parent)
                matches = direct_content_processes(parent, runtime_exe)
                last_probe = (
                    f"expected_pid={expected_pid} expected_start={expected_start} "
                    f"runtime_pid={parent} runtime_exe={runtime_exe!r} "
                    f"matches={matches!r}"
                )
                ...
                last_probe = f"expected_pid={expected_pid} has no parseable parent"
        else:
            last_probe = f"expected_pid={expected_pid} procfs record is absent"
        time.sleep(POLL_SECONDS)
    else:
        fail(state, "external_crash_target_not_unique", last_probe)
