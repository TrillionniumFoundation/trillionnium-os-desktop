use hepta_browser_contracts::TrustedAppIdentity;
use trillionnium_contract_core::DnsLabel;
...
fn shared_origin_vectors_preserve_publisher_isolation() {
    ...
    for line in vectors.lines() {
        ...
        let actual = DnsLabel::parse(publisher.as_str())
            .and_then(|publisher| {
                DnsLabel::parse(app.as_str())
                    .map(|app| TrustedAppIdentity::new(publisher, app).synthetic_origin())
            })
            .ok();
