...
use crate::incarnation::{FixedEntropySource, INCARNATION_ENTROPY_BYTES, IncarnationEntropySource};
struct CountedEntropy {
    calls: Rc<Cell<usize>>,
    mode: u8,
}
impl IncarnationEntropySource for CountedEntropy {
    fn next_entropy(&mut self) -> std::io::Result<[u8; INCARNATION_ENTROPY_BYTES]> {
        self.calls.set(self.calls.get() + 1);
        match self.mode {
            0 => Err(std::io::Error::other("injected entropy failure")),
            1 => Ok([0; INCARNATION_ENTROPY_BYTES]),
            2 => {
                std::thread::sleep(Duration::from_millis(30));
                Ok([3; INCARNATION_ENTROPY_BYTES])
            }
            _ => Ok([4; INCARNATION_ENTROPY_BYTES]),
        }
    }
}
...
fn namespaced_maximum_ordinal_remains_a_valid_opaque_v1_token() {
    let (mut current, _) = actor();
    current.incarnation = incarnation::ActorIncarnation::with_source(FixedEntropySource(
        [0x55; INCARNATION_ENTROPY_BYTES],
    ));
...
fn namespace_is_domain_separated_from_raw_entropy() {
    let mut state = incarnation::ActorIncarnation::with_source(FixedEntropySource(
        [0x55; INCARNATION_ENTROPY_BYTES],
    ));
