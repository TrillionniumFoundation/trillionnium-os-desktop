...
class ServoExactPinWorkflowTests(unittest.TestCase):
    ...
    def test_headed_exact_main_push_binds_current_main_and_event_before(self) -> None:
        runner = (ROOT / "tools/run_servo_headed_runtime_gate.sh").read_text(
            encoding="utf-8"
        )
        ...
        self.assertIn("tested push object is not the current origin/main object", runner)
        self.assertIn('[[ "$tested_sha" == "$current_main" ]]', runner)
        self.assertIn('git merge-base --is-ancestor "${EVENT_BEFORE}" "${parents[0]}"', runner)
        self.assertIn("EVENT_BEFORE:", workflow)
        self.assertIn('require_regular_path "$runtime_example"', runner)
        self.assertIn('require_regular_path "$fixture_example"', runner)
        self.assertIn('--timeout-seconds 120', runner)
