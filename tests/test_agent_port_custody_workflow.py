...
class AgentPortCustodyWorkflowTests(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self.d3_registry_paths = set(
            next(gate for gate in registry["gates"] if gate["id"] == "D3-01")[
                "invalidation_paths"
            ]
        )
    ...
    def test_d3_transitive_experiment_inputs_trigger_both_events(self) -> None:
        """D3 evidence must stale when either Servo experiment changes."""

        self.assertIn("experiments/servo-embedder-probe/**", self.d3_registry_paths)
        self.assertIn("experiments/servo-headed-runtime/**", self.d3_registry_paths)
        self.assertIn(".github/**", self.d3_registry_paths)
        self.assertIn("packaging/debian/**", self.d3_registry_paths)
        for event in ("pull_request", "push"):
            trigger = trigger_paths(self.workflow_text, event)
            for required in (
                "experiments/**",
                ".github/**",
                "packaging/**",
                ...
                self.assertIn(
                    required,
                    trigger,
                    f"D3 transitive input {required!r} is absent from {event}",
                )
