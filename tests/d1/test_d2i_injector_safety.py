...
class D2IInjectorSafetyTests(unittest.TestCase):
    ...
    def test_source_uses_strict_descriptor_backed_state_io(self) -> None:
        ...
        self.assertNotIn("def descendants", source)
        self.assertIn("direct_content_processes", source)
        self.assertIn("default=120.0", source)

    def test_candidate_scan_uses_direct_ppid_when_proc_children_is_unavailable(self) -> None:
        # The hosted runner kernel may omit /proc/*/task/*/children entirely.
        # Model the runtime's direct-child topology and ensure only the exact
        # executable/argv candidate survives the fail-closed filters.
        records = {101: "101", 102: "102", 103: "103"}

        def parent(record: str) -> int:
            return {"101": 42, "102": 99, "103": 42}[record]

        def state(record: str) -> str:
            return {"101": "S", "102": "S", "103": "Z"}[record]

        def exe(pid: int) -> str:
            return {101: "/runtime", 102: "/runtime", 103: "/runtime"}[pid]

        def args(pid: int) -> list[str]:
            return {101: ["runtime", "--content-process"], 102: ["runtime", "--content-process"], 103: ["runtime", "--content-process"]}[pid]

        with (
            mock.patch.object(injector, "proc_pids", return_value=list(records)),
            mock.patch.object(injector, "proc_stat", side_effect=records.__getitem__),
            mock.patch.object(injector, "parent_pid", side_effect=parent),
            mock.patch.object(injector, "proc_state", side_effect=state),
            mock.patch.object(injector, "start_time", return_value=123),
            mock.patch.object(injector, "executable", side_effect=exe),
            mock.patch.object(injector, "cmdline_args", side_effect=args),
        ):
            self.assertEqual(injector.direct_content_processes(42, "/runtime"), [101])
