...
class D1QualificationSeparationTests(unittest.TestCase):
    def test_product_and_d1_qualification_binaries_are_physically_separate(self) -> None:
        ...
        self.assertEqual(
            bins["hepta-agent-port-qualificationd"]["required-features"],
            ["d1-qualification"],
        )
        ...
        self.assertIn('"server" => {', qualification)
        self.assertIn("fn run_server() -> Result<(), FixtureError>", qualification)
        self.assertIn("server mode does not emit a public result", qualification)
        self.assertIn("fn emit_static_report(report: &'static str", qualification)
        self.assertNotIn('println!("{}", result.json)', qualification)
        self.assertIn("qualification_only", qualification)
        self.assertIn("product_handler_connected", qualification)
        self.assertIn("attest_with_static_executable_digest", qualification)
        self.assertIn("hash_trusted_executable", qualification)
        self.assertIn("/usr/libexec/hepta-agent-d1-fixture", qualification)
