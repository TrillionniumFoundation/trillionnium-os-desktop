...
class D1EvidenceSoundnessTests(unittest.TestCase):
    def test_permanent_gate_is_unconditional_read_only_and_non_mutating(self) -> None:
        ...
        self.assertFalse(finalize_d1_evidence.workflow_contains_git_mutation(workflow))

    def test_mutation_scan_rejects_git_option_and_helper_variants(self) -> None:
        for command in (
            "git -C /tmp/repo push origin main",
            "git --git-dir=/tmp/repo update-ref refs/heads/main HEAD",
            "git --work-tree /tmp/repo receive-pack ./incoming",
            "git -C \"/tmp/repo\" \\\n  push origin main",
            "bash -c \"git -C '/tmp/repo' update-ref refs/heads/main HEAD\"",
            "git -C \"/tmp/a;b\" receive-pack /tmp/incoming",
            "/usr/libexec/git-core/git-receive-pack /tmp/repo",
            "git-update-ref refs/heads/main HEAD",
        ):
            self.assertTrue(
                finalize_d1_evidence.workflow_contains_git_mutation(command),
                command,
            )
        for command in (
            "git -C /tmp/repo fetch --depth=1 origin main",
            "git -C /tmp/repo rev-parse HEAD",
            "git -C /tmp/repo status --porcelain=v1",
        ):
            self.assertFalse(
                finalize_d1_evidence.workflow_contains_git_mutation(command),
                command,
