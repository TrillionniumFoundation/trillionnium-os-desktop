...
class D1ProductImageBindingTests(unittest.TestCase):
    ...
    def test_binding_adapter_is_valid_shell_and_never_replaces_qualification_slot(self) -> None:
        ...
        self.assertIn("hepta-agent-port-qualificationd", source)
        self.assertNotIn("legacy_image_slot", source)
        self.assertNotIn('install -m 0755 -- "$product_binary"', source)
        self.assertIn("trap 'rm -rf -- \"$scratch\"' EXIT", source)
        self.assertIn(
            '"$runner" run-pipeline',
            source,
        )
    ...
    def test_guest_product_claim_matches_effective_image_binary(self) -> None:
        ...
        self.assertIn(
            'agent_portd="${D1_AGENT_PORTD_BINARY:-$workspace/target/release/'
            'hepta-agent-portd}"',
            pipeline,
        )
        self.assertIn("D1_AGENT_PORTD_BINARY", pipeline)
        self.assertIn(
