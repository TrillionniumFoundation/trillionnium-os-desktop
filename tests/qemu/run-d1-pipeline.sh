...
# The image input is always the fixture-free product daemon. Qualification and
# fixture binaries are separate inputs and must never be substituted into the
# product slot. Both the D1 wrapper and D2I bind this path explicitly; keeping
# the same default makes direct local invocations fail closed to the product
# artifact as well.
agent_portd="${D1_AGENT_PORTD_BINARY:-$workspace/target/release/hepta-agent-portd}"
if [[ "$agent_portd" != /* ]]; then
  echo "D1 AgentPort binary override must be an absolute path" >&2
  exit 1
fi
...
for binary in "$product_binary" "$qualification_binary" "$agent_fixture"; do
  if [[ ! -f "$binary" || ! -x "$binary" || -L "$binary" ]]; then
    echo "D1 qualification binary is missing or unsafe: $binary" >&2
    exit 1
  fi
done
agent_portd=$(readlink -f -- "$agent_portd")
product_binary=$(readlink -f -- "$product_binary")
qualification_binary=$(readlink -f -- "$qualification_binary")
agent_fixture=$(readlink -f -- "$agent_fixture")
for binary in "$agent_portd" "$product_binary" "$qualification_binary" "$agent_fixture"; do
  if [[ ! -f "$binary" || ! -x "$binary" || -L "$binary" ]]; then
    echo "D1 image binary is missing or unsafe: $binary" >&2
    exit 1
  fi
done
if [[ "$agent_portd" == "$qualification_binary" || "$agent_portd" == "$agent_fixture" ]]; then
  echo "D1 image input resolves to a qualification/fixture binary" >&2
  exit 1
fi
if [[ "$(sha256sum -- "$agent_portd" | awk '{print $1}')" != \
      "$(sha256sum -- "$product_binary" | awk '{print $1}')" ]]; then
  echo "D1 image input is not the exact fixture-free product binary" >&2
  exit 1
fi
if [[ "$(sha256sum -- "$product_binary" | awk '{print $1}')" == \
      "$(sha256sum -- "$qualification_binary" | awk '{print $1}')" ]]; then
  echo "D1 product and qualification binaries unexpectedly have identical bytes" >&2
  exit 1
fi
