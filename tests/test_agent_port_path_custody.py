...
class SystemdMergeSemanticsTests(unittest.TestCase):
    def test_empty_assignment_resets_inherited_list(self) -> None:
        ...
        self.assertEqual(
            VALIDATOR.list_value(assignments, "Service", "ReadWritePaths"),
            ["/state"],
        )
    ...
    def test_last_singleton_assignment_wins(self) -> None:
        ...
        self.assertEqual(
            VALIDATOR.last_value(assignments, "Socket", "SocketUser"), "root"
        )
...
class RepositoryPolicyTests(unittest.TestCase):
    def test_repository_has_root_owned_pathname_custody(self) -> None:
        result = VALIDATOR.validate(ROOT)
        self.assertEqual(result["status"], "PASS_SOURCE_POLICY")
        self.assertEqual(result["socket_owner"], "root")
        self.assertEqual(result["socket_group"], "hepta-agent-socket")
