...
impl RuntimeState {
    ...
    fn trigger_content_crash(self: &Rc<Self>) {
        ...
        let status = match Command::new("/bin/kill")
            .args(["-KILL", &selected.pid.to_string()])
            .status()
        {
            Ok(status) => status,
            Err(error) => {
        ...
        if let Err(error) = fs::write(
            ...
            self.fail(&format!("could not record SIGKILL dispatch: {error}"));
            return;
        }
...
impl WebViewDelegate for RuntimeDelegate {
    ...
    fn notify_crashed(&self, webview: WebView, reason: String, _backtrace: Option<String>) {
        ...
        if !state.signal_sent.get() || state.fault_selected.get().is_none() {
            state.fail("Servo crash callback arrived before attributed fault injection");
            return;
        }
