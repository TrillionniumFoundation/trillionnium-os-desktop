...
//! codec, connected AgentPort, and session self-check only: it does not bind a
//! listener, dispatch a BrowserActor, start Servo, or perform a network effect.
...
mod tests {
    ...
    #[test]
    fn self_check_exercises_transport_codec_agent_port_and_session_core() {
        ...
        assert!(report.to_json().contains(IMPLEMENTATION_STAGE));
    }
}
